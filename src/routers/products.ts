...
// create
routerProducts.post("/api/products", (req: Request, res: Response) => {
...
// update
routerProducts.put("/api/products/:id", (req: Request, res: Response) => {
  const productId = req.params.id;

  const { title, price, category, description, image } = req.body;

  client.query(
    "UPDATE products SET title = $1, price = $2, category = $3, description = $4, image = $5 WHERE id = $6",
...
// delete
routerProducts.delete("/api/products/:id", (req: Request, res: Response) => {
  const productId = req.params.id;
